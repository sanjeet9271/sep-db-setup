...
-- Tables affected:
--   1. case_drafts (draft_id) - 7 hex chars (~268M combinations for 250K records)
--   2. draft_attachments (attachment_id) - datt_ + 8 hex chars (~4.3B for 12.5M records)
--   3. case_attachments (attachment_id) - catt_ + 8 hex chars (~4.3B for 100M records)
...
-- ============================================================================
-- Generates 7-character hex IDs (uppercase, no prefix)
-- Format: A3F8E2C, C91D4F7, 2B7E9A6
-- Character set: 0-9, A-F (16 characters)
-- Combinations: 16^7 = 268,435,456
-- ============================================================================
...
-- ============================================================================
-- Generates attachment IDs with format: datt_XXXXXXXX (8 hex chars after prefix)
-- Format: datt_a3f8e2c9, datt_c91d4f87, datt_2b7e9a16
-- Character set: 0-9, a-f (16 characters, lowercase)
-- Combinations: 16^8 = 4,294,967,296 (4.3 billion)
-- Note: 'datt_' prefix = draft attachment (industry standard)
-- ============================================================================
...
COMMENT ON FUNCTION generate_draft_attachment_id() IS 'Generates unique draft attachment ID: datt_XXXXXXXX (4.3B combinations for 12.5M records)';

-- ============================================================================
-- 3. UPDATE: generate_attachment_id() for case_attachments table
-- ============================================================================
-- Generates attachment IDs with format: catt_XXXXXXXX (8 hex chars after prefix)
-- Format: catt_a3f8e2c9, catt_c91d4f87, catt_2b7e9a16
-- Character set: 0-9, a-f (16 characters, lowercase)
-- Combinations: 16^8 = 4,294,967,296 (4.3 billion)
-- Note: 'catt_' prefix = case attachment (industry standard)
-- ============================================================================
...
COMMENT ON FUNCTION generate_case_attachment_id() IS 'Generates unique case attachment ID: catt_XXXXXXXX (4.3B combinations for 100M records)';
...
COMMENT ON COLUMN draft_attachments.attachment_id IS 'Auto-generated unique ID: datt_XXXXXXXX (draft attachment) using UUID';
COMMENT ON COLUMN case_attachments.attachment_id IS 'Auto-generated unique ID: catt_XXXXXXXX (case attachment) using UUID';
...
DO $$
DECLARE
    test_draft_id varchar(7);
    test_draft_attachment_id varchar(13);
    test_case_attachment_id varchar(13);
...
BEGIN
    ...
    RAISE NOTICE '  Draft ID:              % (7 hex chars)', test_draft_id;
    RAISE NOTICE '  Draft Attachment ID:   % (datt_ + 8 hex chars)', test_draft_attachment_id;
    RAISE NOTICE '  Case Attachment ID:    % (catt_ + 8 hex chars)', test_case_attachment_id;
    RAISE NOTICE '  Comment ID:            % (cmt_ + 8 hex chars)', test_comment_id;
    RAISE NOTICE '=================================================================';
    RAISE NOTICE 'Capacity Summary:';
    RAISE NOTICE '  Draft IDs:        268M combinations for 250K expected records';
    RAISE NOTICE '  Draft Attach:     4.3B combinations for 12.5M expected records (datt_)';
    RAISE NOTICE '  Case Attach:      4.3B combinations for 100M expected records (catt_)';
